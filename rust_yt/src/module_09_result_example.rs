pub fn result_basics_example() {
    ...
    let failed_transaction: Result<u64, &str> = Err("Transaction failed due to insufficient funds.");

    match success_transaction {
        Ok(amount) => println!("Transaction succeeded with {} tokens.", amount),
        Err(e) => println!("Transaction failed with {}.", e),
    }
...
pub fn unwrap_or_example()
{
    ...
    println!("Transaction failed, default to: {} tokens", amount_or_default);
}
